$(async function () {
  ...
  const $navWelcome = $("#nav-welcome"); // davery
  const $userProfile = $("#user-profile"); // davery
  const $favoriteArticles = $("#favorited-articles"); // davery
  const $errors = $("#errors"); // davery
  const $editProfileForm = $("#edit-profile-form"); // davery
  ...
  /* Check for errors returned from API call */
  function errorsFound(obj) {
    if (obj.hasOwnProperty("error")) {
      $errors.text(obj.error).slideDown();
      setTimeout(function () {
        $errors.text("").slideUp();
      }, 3000);
      return true;
  ...
  /* Get user profile */
  $("#nav-user-profile").on("click", async (e) => {
    e.preventDefault();
    hideElements([
      $favoriteArticles,
      $submitForm,
  ...
  /* User clicks button to edit profile */
  $("#edit-profile-button").on("click", function () {
    $userProfile.find("#userProfileTitle").hide();
    $userProfile.find("section").hide();
    $editProfileForm.show();
    $editProfileForm.find("#editName").val(currentUser.name);
    $editProfileForm.find("#editPassword").attr("placeholder", "new password");
  });

  /* User updates profile */
  $editProfileForm.on("submit", async (e) => {
    e.preventDefault();
    const { editName, editPassword } = e.target.elements;
    ...
    if (updated) {
      currentUser = await getCurrentUser();
      $userProfile.find("#profile-name").text(`Name: ${currentUser.name}`);
    }
    $editProfileForm.hide();
    $userProfile.find("h4").text("User Profile Info");
    $userProfile.find("section").show();
  });
  ...
  const checkFavorite = async (e) => {
    ...
    if (favorite) {
      // get the updated user w/ favorites - davery
  ...
  function hideElements(other = []) {
    ...
    if (other.length) other.forEach((item) => elementsArr.push(item));
